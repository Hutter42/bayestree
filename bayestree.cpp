...
- ModelDim()            Compute probability of Model Dimension k for no Data or
                          single data item
- BayesTree()           Computes Quantities under Tree Mixture Model
- EvalBayesTree()       Compute Predictive distribution given data D and other
                          Estimators
- PrintWeightTable()    Prints weight table to device dev
- PrintBayesTree()      Evaluate and Print BayesTree
- PrintBT4Manyn()       Calls PrintBayesTree() for data size n=0,1,10,1000,...,
                          nmax
...
#define rrrand()                                                         \
  ((((rand() / (RAND_MAX + 1.0) + rand()) / (RAND_MAX + 1.0) + rand()) / \
    (RAND_MAX + 1.0)))
 ...
 *-MH-------------------------------------------------------------------------*/
int panicf(const char *s)
/*----------------------------------------------------------------------------*/
{
  //  fprintf(dev,s);
  printf("%s", s);  // set a breakpoint here
  return 0;
}  // End of panicf()
...
/*----------------------------------------------------------------------------*/
{
  double lng = 0;
  if (((int)n) != n)
    panicf("lnGamma() not yet implemented for non-integer n\n");
...
/*-----------------------------------------------------------------------------*
  ...
  Remark:       Currently only rejection sampling for bounded distributions is
                  implemented
 *-MH-------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------*
  ...
  Parameters:   x = in [0,1) point at which function f shall be evaluated
                Global DistrParam[] = vector of parameters, function knows how
                  many to take
...
/*----------------------------------------------------------------------------*/
{
  ...
  if (mode == 2) return fmax(b / a, (1 - b) / (1 - a));
  if (mode == 1) return x < a ? b / a : (1 - b) / (1 - a);
  if (mode == 3) {
    fprintf(dev, "Jump(%f,%f)", a, b);
    return 0;
  }
...
/*-----------------------------------------------------------------------------*
  ...
  Parameters:   Beta(a[0],a[1])= x^a[0] * x^a[1] / Beta(a[0],a[1]) (e.g. a[0]=3,
                  a[1]=6)
...
/*-----------------------------------------------------------------------------*
  Description:  Compute probability of Model Dimension k for no Data or single
                  data item
...
/*-----------------------------------------------------------------------------*
  ...
  Parameters:   in: D[0..n-1] = data points in [0,1]
                ...
                in: x = point at which expected height E[h(x)|D] shall be
                          computed.
 ...
 *-MH-------------------------------------------------------------------------*/
void BayesTree(double D[], int n, double &lnpD, int l, double x, double &h,
...
/*----------------------------------------------------------------------------*/
{
  ...
  if (l >= 0 && (n == 0 || D[0] == x || x < 0 ||
                 x >= 1))  // test: l>#, result independent of #
  {
    if (n <= 1) {
      if (x >= 0 && x < 1) h = 1;
      lnpD = log(1.0);
    ...
    if (i == n) {
      if (n == 2) {  // printf("Two identical points\n");
        lnpD = log(1.5);
        hav = 2;
        if (x >= 0 && x < 1) h = 2;
  ...
  double *D0 =
      (double *)malloc(n * sizeof(double));  // internal sort on D possible
...
/*-----------------------------------------------------------------------------*
  Description:  Compute Predictive distribution given data D and other
                  Estimators
...
/*----------------------------------------------------------------------------*/
{
  int nrx, mx;
  // double hav2 = 0
  double dummy, lnpDx, havx, norm = 0;
  ...
  for (int i = 0; i < d; i++) {
    ...
    // p(x,x|D)
    if (pDxx) {
      BayesTree(D, n + 2, lnpDx, 0, x, dummy, havx, 0, 0, nrx, mx);
      pDxx[i] = exp(lnpDx - lnpD);
    }
    // height w/o x is correct
    if (ha) {
      BayesTree(D, n, lnpDx, 0, x, ha[i], havx, 0, 0, nrx, mx);
      if (fabs(havx - hav) > 1e-9 || mx != mult || fabs(lnpDx - lnpD) > 1e-9)
        panicf(
            "Inconsistency in AvHeight or Multiplicity in EvalBayesTree()\n");
    }
  }
  if (pDx && fabs(norm - 1) > 7.0 / d)
    panicf("Normalization Error in EvalBayesTree()\n");
  if (ha) ha[d] = hav;
  if (pDx) pDx[d] = norm;
  if (pDD)  // p(D[i]|D)
  {
    printf("\nComputing p(D[i]|D) for i=0..%d ", min(n, maxnprint) - 1);
    for (int i = 0; i < min(n, maxnprint); i++) {
      printf("*");
      double x = D[n] = D[i];
      BayesTree(D, n + 1, lnpDx, 0, x, dummy, havx, 0, 0, nrx, mx);
...
/*----------------------------------------------------------------------------*/
{
  ...
  for (int i = 0; i < imax; i++) {
    ...
    for (double x = 0; x <= 1 + step / 2; x += step)
      fprintf(dev, "%2.9f\t", exp(lnWeight(fmax(0, (1 - x)) * n, x * n)));
    fprintf(dev, "\n");
  }
...
{  // Initialization
  ...
  double *D = (double *)malloc(
      (n + 5) * sizeof(double));  // +k important for k-th moments
  double *pDx =
      (double *)malloc((d + 1) * sizeof(double));  // last element used for sum
  double *pDxx =
      (double *)malloc((d + 1) * sizeof(double));  // last element used for sum
  double *pDD = (double *)malloc((min(n, maxnprint)) * sizeof(double));
  double *ha =
      (double *)malloc((d + 1) * sizeof(double));  // last element used for sum
  double *pN = (double *)malloc((Nmax + 1) *
                                sizeof(double));  // last element used for sum
  printf("\nBayes Tree for sample size n=%d",
         n);  // Distr(0,3); goes to BayesTree.dat
  fprintf(dev, "\nBayes Tree for ");
  Distr(0, 3);
  fprintf(dev, " Function for sample size n=%d", n);
  fflush(dev);
  fprintf(dev, "\n");
  ...
  fprintf(dev, "\nn=%d\tData       \t", n);
  for (i = 0; i < n; i++)
    if (i < maxnprint) fprintf(dev, "%3.15lf\t", D[i]);
  // Print first p(x|D) for those points
  fprintf(dev, "\nD for n=%d\tp(Data|D)  \t", n);
  for (i = 0; i < n; i++)
    if (i < maxnprint) fprintf(dev, "%3.15lf\t", pDD[i]);
  // Print scale
  fprintf(dev, "\n");
  fprintf(dev, "\nd=%d x\t         \t", d);
  ...
  fprintf(dev, "\nExact\tDistr(x)   \t");
  for (i = 0; i < d; i++)
    fprintf(dev, "%3.15lf\t", Distr(x = (i + 0.5) / d, 1));
  fprintf(dev, "\n");
  ...
  fprintf(dev, "\nn=%d\tDimension  \t", n);
  for (i = 0; i < Nmax; i++) {
    if (i < maxnprint) fprintf(dev, "%d\t", i);
  }
  fprintf(dev, "\nn=%d\tProb[Dim]  \t", n);
  for (i = 0; i < Nmax; i++) {
    if (i < maxnprint) fprintf(dev, "%3.15lf\t", pN[i]);
  }
  fprintf(dev, "\nn=%d\tVar(q(x)|D)\t", n);
  for (i = 0; i < d; i++)
    fprintf(dev, "%3.15lf\t", sqrt(pDxx[i] - pDx[i] * pDx[i]));
 ...
 *-MH-------------------------------------------------------------------------*/
int PrintBT4Manyn(DistrType Distr, const char *fname, int nmax, int Nmax, int d,
...
/*----------------------------------------------------------------------------*/
{
  ...
  fprintf(dev, "\nBayes Tree for ");
  Distr(0, 3);
  fprintf(dev, " Function for various sample sizes n");
  fprintf(dev,
          "\n=================================================================="
          "===");
  fprintf(dev, "\n\nn=0&1\t");
  ...
  srand(12345);
  PrintBayesTree(Distr, 0, Nmax, d);
  for (int n = 1; n <= nmax; n *= 10) {
    srand(12345);
    PrintBayesTree(Distr, n, Nmax, d);
  }
  fclose(dev);
  return 0;
...
/*----------------------------------------------------------------------------*/
{
  int d = 200, nmax = 100000,
      Nmax = 200;  // 2min for n=10000 per PrintBT4Manyn()

  // Beta(3,6) Distribution (prototype for smooth distribution
  PrintBT4Manyn(BetaDistr, "/tmp/BTBeta.dat", nmax, Nmax, d, 3, 6, 0);
  // Linear Distribution q(x)=2x (prototype for continuos distribution
  PrintBT4Manyn(LinearDistr, "/tmp/BTLinear.dat", nmax, Nmax, d, 0, 0, 0);
  // Jump at non-binary fraction 1/3: prototype for infinite dimension
  PrintBT4Manyn(JumpDistr, "/tmp/BTJump.dat", nmax, Nmax, d, 1 / 3.0, 0.5, 0);
  // Distribution q(x)=2/sqrt(x): prototype for proper singular distribution
  PrintBT4Manyn(SingularDistr, "/tmp/BTSingular.dat", nmax, Nmax, d, 0, 0, 0);
  // Jump at non-binary fraction 1/2: prototype for finite dimension
  PrintBT4Manyn(JumpDistr, "/tmp/BTJump05.dat", nmax, Nmax, d, 0.5, 0.9, 0);
  // Uniform Dsitribution
  PrintBT4Manyn(JumpDistr, "/tmp/BTUniform.dat", nmax, Nmax, d, 0.5, 0.5, 0);
  return 0;
}  // End of TestBayesTree()
